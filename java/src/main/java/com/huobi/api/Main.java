...
import com.huobi.api.indicator.Indicators;
...
public class Main {
  ...
  static void apiSample() {
    ...
    print(accounts);
    Long accountId = accounts.stream().filter(a -> a.type.equals("spot")).map(a -> a.id).findFirst().get();
...
//      print(r);
//    }
    KlineRequest req = new KlineRequest();
    req.setPeriod("15min");
    req.setSymbol("etcusdt");
    req.setSize("30");
    Klines kls = client.getKlines(req);
    List<Kline> klines = kls.getKlines();
    long ts = kls.getTs();
    System.out.println(klines);
    double[] highPrices = ArrayUtils.toPrimitive(klines.stream().map(a -> Double.valueOf(a.getHigh())).toArray(Double[]::new));
    double[] lowPrices = ArrayUtils.toPrimitive(klines.stream().map(a -> Double.valueOf(a.getLow())).toArray(Double[]::new));
    double[] closePrices = ArrayUtils.toPrimitive(klines.stream().map(a -> Double.valueOf(a.getClose())).toArray(Double[]::new));
    
    double[] cci = Indicators.getInstance().cci(highPrices, lowPrices, closePrices, 14);
    int ii = 0;
    DefaultCategoryDataset ds = new DefaultCategoryDataset();
    for (double d : cci) {
        System.out.println(d);
        ds.addValue(d, "cci", ii++ + "");
    }
    

    
//    ChartFrame cf = new ChartFrame("test", new CreateJFreeChart().makeLineAndShapeChart("cci60", "time", "cci", ds));
//    cf.pack();
//    cf.setVisible(true);
    ...
    TimeSeries cciseries=new TimeSeries("");//对应时间成交量数据
   
    for (int i = klines.size() -1; i>-1; i--) {
        Kline k = klines.get(i);
        System.out.println(k.getId() + "/" + new Date(ts));
        series.add(new Minute(new Date(ts)), Double.valueOf(k.getOpen()), Double.valueOf(k.getHigh()), Double.valueOf(k.getLow()), Double.valueOf(k.getClose()));
        series2.add(new Minute(new Date(ts)), Double.valueOf(k.getVol()));
        cciseries.add(new Minute(new Date(ts)), cci[i]);
        ts -= 15*60*1000;
    }
    ...
    TimeSeriesCollection ccitimeSeriesCollection=new TimeSeriesCollection();//保留成交量数据的集合
    ccitimeSeriesCollection.addSeries(cciseries);
...
//    x1Axis.setTickUnit(new DateTickUnit(DateTickUnit.DAY,7));//设置时间刻度的间隔，一般以周为单位
    x1Axis.setDateFormatOverride(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss"));//设置显示时间的格式
    NumberAxis y1Axis=new NumberAxis("Price");//设定y轴，就是数字轴
    ...
    NumberAxis y2Axis=new NumberAxis("Vol");//设置Y轴，为数值,后面的设置，参考上面的y轴设置
    ...
    XYPlot plot3=new XYPlot(ccitimeSeriesCollection,null,new NumberAxis("CCI"),xyBarRender);
    ...
    combineddomainxyplot.add(plot1, 2);//添加图形区域对象，后面的数字是计算这个区域对象应该占据多大的区域2/3
       ...
       ChartFrame frame = new ChartFrame("etcustd", chart);
    frame.pack();
